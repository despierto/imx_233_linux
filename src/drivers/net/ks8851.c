...
struct ks8851_net {
    ...
    struct task_struct  *ks8851_irq_thread_task;
 ...
 * current status by scheduling ks8851_irq_thread().
 */
...
/**
 * ks8851_irq_thread - work queue handler for dealing with interrupt requests
 ...
 */
static int ks8851_irq_thread(void *param)
{
    struct ks8851_net *ks = (struct ks8851_net *)param;
    unsigned status;
 ...
 */
static int ks8851_start_xmit(struct sk_buff *skb,
...
static int __devinit ks8851_probe(struct spi_device *spi)
{
    ...
    dev_info(&spi->dev, "revision %d, MAC %pM\n",
         CIDER_REV_GET(ks8851_rdreg16(ks, KS_CIDER)),
         ndev->dev_addr);

    ks->ks8851_irq_thread_task = kthread_run(ks8851_irq_thread, ks, "ks8851");
    ret = -EIO;
    if(IS_ERR(ks->ks8851_irq_thread_task))
        goto err_netdev;
