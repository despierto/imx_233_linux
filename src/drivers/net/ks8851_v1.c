...
static void ks8851_restart_work(struct work_struct *work)
{
    struct ks8851_net *ks = container_of(work, struct ks8851_net, restart_work);
    struct net_device *ndev = ks->netdev;
    int ret;

    rtnl_lock();
    if (netif_running(ndev)) {
        ks8851_net_stop(ndev);
        ret = ks8851_net_open(ndev);
        if (unlikely(ret)) {
...
static int __devinit ks8851_probe(struct spi_device *spi)
{
    ...
    INIT_WORK(&ks->restart_work, ks8851_restart_work);
