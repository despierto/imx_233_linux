...
#define TS_GPIO_PENIRQ PINID_GPMI_CE1N
//#define TS_GPIO_PENIRQ PINID_PWM2
...
{
	if(gpio_request(TS_GPIO_PENIRQ, "ADS7843 pendown") < 0)
	    printk(KERN_INFO "Can't get ADS7843 pen down GPIO\n");
        gpio_direction_input(TS_GPIO_PENIRQ);
        set_irq_type(gpio_to_irq(TS_GPIO_PENIRQ),IRQ_TYPE_EDGE_FALLING);
	return gpio_to_irq(TS_GPIO_PENIRQ);
}

static int ads7846_pendown_state(void)
{
	return !gpio_get_value(TS_GPIO_PENIRQ);	/* Touchscreen PENIRQ */
}
...
static struct spi_gpio_platform_data imx_spi_gpio_platform_data = {
	.sck = PINID_LCD_DOTCK,
	.mosi = PINID_GPMI_WPN,
...
static struct platform_device imx_spi_gpio_device = {
	.name = "spi_gpio",
	.id = 3,
	.dev = {
	    .platform_data = &imx_spi_gpio_platform_data,
	    },
}; 
#endif
...
static struct pin_desc lcd_lms430_desc[] = {
	{ PINID_LCD_D00, PIN_FUN1, PIN_8MA, PIN_3_3V, 0 },
	{ PINID_LCD_D01, PIN_FUN1, PIN_8MA, PIN_3_3V, 0 },
	{ PINID_LCD_D02, PIN_FUN1, PIN_8MA, PIN_3_3V, 0 },
	{ PINID_LCD_D03, PIN_FUN1, PIN_8MA, PIN_3_3V, 0 },
...
//		.modalias       = "spidev",
		...
		.bus_num	= 3,
		.controller_data  = PINID_SSP1_DATA2,
